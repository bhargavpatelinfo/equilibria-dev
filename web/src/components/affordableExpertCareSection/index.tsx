              ...
              {imageTitleDescriptionCtas?.map((item, index) => {
                ...
                return (
                  <div key={index} className="flex flex-col gap-6 justify-normal">
                    {image && (
                      <div className="">
                        <Image src={image} alt={image?.alt} className="aspect-[464/534] rounded-xxl object-cover"/>
                      </div>
                    )}
                    <div className="flex flex-col gap-4">
                      {title && <h3 className="text-32 lg:text-40 font-semibold tracking-[-0.8px] font-Kulim text-darkPurple leading-[110%]">{title}</h3>}
                      {description?.custom_rich_text && (
                        <div className="[&>p]:text-lg em:[&>p]:text-xl [&>p]:text-darkPurple [&>p]:tracking-[-0.1px] [&>p]:font-semibold [&>p]:opacity-70 [&>p>span]:text-26 em:[&>p>span]:text-32 lg:[&>p>span]:text-44 [&>p>span]:pb-4 [&>p>span]:leading-[100%] [&>p>span]:block [&>p>span]:tracking-[-0.22px]">
                          <RichText block={description} />
                        </div>
                      )}
                      </div>
